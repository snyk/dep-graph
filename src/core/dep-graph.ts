...
class DepGraphImpl implements types.DepGraphInternal {
  ...
  public pkgPathsToRoot(pkg: types.Pkg): types.PkgInfo[][] {
    ...
    for (const nodeId of this.getPkgNodeIds(pkg)) {
      pathsToRoot.push(...this.pathsFromNodeToRoot(nodeId));
    }
  ...
  private pathsFromNodeToRoot(
    ...
    for (const id of parentNodesIds) {
      allPaths.push(
        ...this.pathsFromNodeToRoot(id, ancestors.concat(nodeId)).map((path) =>
          [pkgInfo].concat(path),
        ),
      );
    }
  ...
  private countNodePathsToRoot(
    ...
    const count = parentNodesIds.reduce((acc, parentNodeId) => {
      return (
        acc + this.countNodePathsToRoot(parentNodeId, ancestors.concat(nodeId))
      );
    }, 0);
